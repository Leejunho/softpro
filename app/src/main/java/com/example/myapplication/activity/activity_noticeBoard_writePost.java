...
public class activity_noticeBoard_writePost extends activity_main_basic {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_noticeboard_writepost);
    ...
    View.OnClickListener onClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                case R.id.button_send:  // 물품등록 버튼 클릭했을때 동작
                    post_add();
                    break;
    ...
    private void post_add() {
        ...
        if(p.length() < 0) {
            showToast(activity_noticeBoard_writePost.this, "가격 정보를 입력해 주세요");
            return;
        }
        else if(p.length() > 9){
            showToast(activity_noticeBoard_writePost.this, "가격 정보를 확인해 주세요");
            return;
        }
        String p2 = ((EditText) findViewById(R.id.textView_boxnum)).getText().toString();
        if(p2.equals("") || p2.length() < 0) {
            showToast(activity_noticeBoard_writePost.this, "택배함 번호를 입력해 주세요");
            return;
        }
        else if(p2.length() > 9) {
            showToast(activity_noticeBoard_writePost.this, "택배함 번호를 확인해 주세요");
            return;
        }
                                            ...
                                            if (document2.exists()) {
                                                ...
                                                if(document2.getData().get("telephone").toString().equals(document.getData().get("telephone").toString())) {
                                                    if (title.length() > 0 && price >= 0 && term.length() > 0) {
                                                        ...
                                                        String complete = "NO";
                                                        String checkpublisher = "NO";
                                                        String checkconsumer = "NO";
                                                        if (postInfo != null) {
                                                            ...
                                                            checkpublisher = postInfo.getCheckpublisher();
                                                            checkconsumer = postInfo.getCheckconsumer();
                                                        }

                                                        // db 저장 함수

                                                        uploader(documentReference, new postInfo(title, price, term, contents, user.getUid(), new Date(), viewCount, consumerblank, roomidblank, completePublisher, completeComsumer, complete, boxnum, checkpublisher, checkconsumer));
                                                    }
                                                    else {
                                                        showToast(activity_noticeBoard_writePost.this, "정보를 입력해 주세요");
                                                    }
                                                }
                                                else {
                                                    showToast(activity_noticeBoard_writePost.this, "본인의 택배함 번호를 입력해 주세요");
                                                }
                                            }
                                            else {
                                                showToast(activity_noticeBoard_writePost.this, "택배함 번호를 확인해 주세요");
        ...
        documentReference.set(postInfo.getPostInfo())
            .addOnSuccessListener(new OnSuccessListener<Void>() {
                ...
                public void onSuccess(Void aVoid) {
                    ...
                    showToast(activity_noticeBoard_writePost.this,"게시글 등록에 성공하셨습니다");
                    // view count 새로 갱신
                    updateviewcount();
            ...
            .addOnFailureListener(new OnFailureListener() {
                ...
                public void onFailure(@NonNull Exception e) {
                    loaderLayout.setVisibility(View.GONE);
                    showToast(activity_noticeBoard_writePost.this,"게시글 등록에 실패하셨습니다");
                }
            });
    }
    ...
    private void postInit() {
        if(postInfo != null) {
            ...
            ((EditText) findViewById(R.id.textView_boxnum)).setText(String.valueOf(postInfo.getBoxnum()));
        }
    }
    ...
    private void userInit() {
        if(user != null) {
            ...
            showToast(activity_noticeBoard_writePost.this, "잘못된 접근입니다.");
            finish();
        }
    }
